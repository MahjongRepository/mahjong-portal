...
class RatingRRCalculation(object):
    ...
    def calculate_players_rating_rank(self, rating, rating_date):
        ...
        for coefficient in coefficient_temp:
            ...
            if coefficient.tournament_id not in stages_tournament_ids:
                value = self._calculate_percentage(
                    float(coefficient.coefficient), coefficient.age
                )
                coefficients.append(
                    {
            ...
            if stage_tournament_id in tournament_ids:
                ...
                if tournament.end_date >= rating_date:
                    ...
                    stage_coefficients = list(
                        set(HARDCODED_COEFFICIENTS[stage_tournament_id].values())
                    )
                    for x in stage_coefficients:
                        value = self._calculate_percentage(x, age)
                        coefficients.append(
                            {"coefficient": x, "age": age, "value": value}
                        )
        ...
        for player in self.players:
            ...
            if num_tournaments == total_tournaments:
                ...
                for tournaments_results_option in itertools.combinations(
                    deltas, num_tournaments
                ):
    ...
    def calculate_players_deltas(self, tournament, rating, rating_date):
        ...
        results = TournamentResult.objects.filter(tournament=tournament).filter(
    ...
    def tournament_coefficient(self, tournament):
        ...
        """
        return self.players_coefficient(tournament) + self.sessions_coefficient(
            tournament
        )
    ...
    def calculate_rating_delta(
        self, tournament_result, rating_date, tournament, player
    ):
        """
        Determine player delta and tournament properties
        """
        tournament_coefficient = self.tournament_coefficient(tournament)
        tournament_coefficient = get_tournament_coefficient(
            self.IS_EMA, tournament.id, player, tournament_coefficient
        )
    ...
    def sessions_coefficient(self, tournament):
        ...
        if sessions > 20:
            ...
            if sessions <= 8:
                ...
                calculated += (
                    8 * first_value + 4 * second_value + (sessions - 12) * third_value
                )
            elif 16 < sessions <= 20:
                calculated += (
                    8 * first_value
    ...
    def _calculate_player_rating(
        ...
        best_results = sorted(deltas, key=lambda x: x.delta, reverse=True)[
        ...
        for x in selected_coefficients:
            max_coefficient_calculation.append(
                "{} * {}".format(
                    floatformat(x["coefficient"], -2), floatformat(x["age"] / 100, -2)
                )
            )
