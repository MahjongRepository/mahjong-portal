...
class TenhouCalculatorTestCase(TestCase):
    ...
    def test_calculations(self):
        game_records = self._create_game_record(
            self._generate_game_records([1, 1, 2, 3, 1, 1, 4, 2])
        )
    ...
    def test_hanchan_and_tonpusen(self):
        game_records = self._create_game_record(
            [
                {
                    "date": timezone.now() + timedelta(seconds=0),
                    "game_rules": u"四般東",
                    "place": 1,
                },
                {
                ...
                {
                    "date": timezone.now() + timedelta(seconds=2),
                    "game_rules": u"四般東",
                    "place": 1,
                },
                {
                    "date": timezone.now() + timedelta(seconds=3),
                    "game_rules": u"四般東",
                    "place": 1,
                },
                {
                    "date": timezone.now() + timedelta(seconds=4),
                    "game_rules": u"四般東",
                    "place": 1,
                },
            ]
    ...
    def test_old_and_new_kyu_lobby_append_points(self):
        game_records = self._create_game_record(
            [
                {
                    "date": datetime(2017, 10, 23, tzinfo=pytz.utc),
                    "game_rules": u"四般南",
                    "place": 2,
                },
    ...
    def test_you_cant_lose_1_kyu(self):
        ...
        four_places = [4] * 19
        game_records = self._create_game_record(
            self._generate_game_records(first_places + four_places)
        )
        result = FourPlayersPointsCalculator.calculate_rank(game_records)
