...
class ParseLogLinesTestCase(TestCase):
    ...
    def test_parse_line(self):
        ...
        self.assertEqual(results['game_type'], u'四上南喰赤－')
        self.assertIsNotNone(results['hash'])

    def test_parse_line_with_brace_symbol_in_name(self):
        line = u'23:56 | 11 | 三般東喰赤－ | 烏龍茶(黒)(+43.0) misery20(-7.0) NoName(-36.0)'

        results = parse_log_line(line)
        self.assertEqual(results['players'][0]['player'], u'烏龍茶(黒)')
        self.assertEqual(results['players'][1]['player'], u'misery20')
        self.assertEqual(results['players'][2]['player'], u'NoName')
